...
Refactored to use modular components - now imports from:
- illustration_service.py - Illustration generation
- diversity_selector.py - Candidate diversity selection
- orchestrator_slim.py - Core orchestration logic
"""

# Re-export main functions from the slim orchestrator
# Re-export supporting modules for backwards compatibility
from .diversity_selector import select_diverse_candidates as _select_diverse_candidates
from .illustration_service import (
    ConceptSectionMatch,
    IllustrationResult,
    IllustrationService,
)
from .orchestrator_slim import (
...
def generate_single_article(
    ...
    try:
        ...
        # Note: Using Mermaid for all diagram types (better rendering, dark mode support, dev-friendly)
        CONCEPT_TO_FORMAT = {
            "network_topology": [
                "ascii",
                "mermaid",
            ...
            "system_architecture": [
                "mermaid",
                "ascii",
            ],  # Mermaid for visual complexity (better rendering than SVG)
            "data_flow": [
                "mermaid",
                "ascii",
            ...
            "scientific_process": [
                "mermaid",
                "ascii",
            ],  # Mermaid for sequences, ASCII for detailed methodology
                    ...
                    if concept_names:
                        ...
                        if suitable_sections:
                            ...
                            text_selector = TextIllustrationQualitySelector(
                                client,
                                n_candidates=getattr(
                                    config, "text_illustration_candidates", 3
                                ),
                            ...
                            for match in top_matches:
                                try:
                                    ...
                                    fallback_formats = CONCEPT_TO_FORMAT.get(
                                        concept, ["mermaid"]
                                    )
                                    ...
                                    if selected_format == "ascii":
                                        ...
                                        if (
                                            diagram
                                            and diagram.quality_score
                                            >= text_selector.quality_threshold
                                        ):
                                            console.print(
                                    ...
                                    # If ASCII failed or not selected, use Mermaid generator
                                    if diagram is None:
                                    ...
                                    if diagram and injected_content:
                                        ...
                                        if selected_format == "ascii":
                                            ...
                                            # Strip any existing fences (GPT sometimes includes them)
                                            mermaid_content = diagram.content.strip()

                                            # Remove leading code fences (handle multiple cases)
        ...
        final_content = injected_content if injected_content is not None else content

        # Format markdown to fix formatting issues (headings, spacing, etc.)
