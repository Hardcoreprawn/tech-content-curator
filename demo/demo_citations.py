...
laid groundwork for these investigations."""
    ...
    for i, cit in enumerate(citations, 1):
        confidence_str = "{}%".format(int(cit.confidence * 100))
        print("  {}. '{}' ({}, confidence: {})".format(
            i, cit.original_text, cit.authors, confidence_str))
    ...
    for citation in citations:
        print("\nResolving: {} ({})".format(citation.original_text, citation.year))
    ...
    for citation, resolved in zip(citations, resolved_list):
        if resolved:
    ...
    print("Extracted: {} citations (including variations)".format(len(citations)))
