...
class TestCoverImageSelector:
    ...
    def test_search_queries_generated_via_llm(self, selector):
        """Verify that search queries are generated via LLM."""
        with patch("src.images.selector.chat_completion") as mock_chat:
            mock_chat.return_value = self._chat_response(
                '{"unsplash": "flying bird", "pexels": "bird", "dalle": "a bird flying in the sky"}'
            )
    ...
    def test_search_queries_fallback_when_json_invalid(self, selector):
        """Fallback to defaults when LLM returns invalid JSON."""
        with patch("src.images.selector.chat_completion") as mock_chat:
            mock_chat.return_value = self._chat_response("not valid json")
    ...
    def test_dalle_fallback_image_cost(self, selector):
        """Test DALL-E fallback image has correct cost."""
        with patch("src.images.selector.create_image") as mock_create_image:
            mock_create_image.return_value = SimpleNamespace(
                data=[SimpleNamespace(url="https://openai.com/image.png")]
            )
    ...
    @patch("src.images.selector.httpx.get")
    def test_select_uses_free_sources_before_dalle(self, mock_get, selector):
        """Test selection strategy tries free sources before DALL-E."""
        with (
            patch("src.images.selector.chat_completion") as mock_chat,
            patch("src.images.selector.create_image") as mock_create_image,
        ):
            # Mock LLM query generation AND validation (2 calls)
            mock_chat.side_effect = [
                self._chat_response(
                    '{"wikimedia": "bird", "unsplash": "bird", "pexels": "bird", "dalle": "bird"}'
                ),
                self._chat_response("yes"),
            ]
    ...
    @patch("src.images.selector.httpx.get")
    def test_select_falls_back_to_dalle_when_free_fail(self, mock_get, selector):
        ...
        with (
            ...
            mock_chat.return_value = self._chat_response(
                '{"wikimedia": "bird", "unsplash": "bird", "pexels": "bird", "dalle": "bird drawing"}'
            )

            # Mock all free sources failing
            mock_get.side_effect = TimeoutError("All APIs timeout")
            mock_create_image.return_value = SimpleNamespace(
                data=[SimpleNamespace(url="https://openai.com/bird.png")]
            )
    ...
    @patch("src.images.selector.httpx.get")
    def test_select_respects_quality_thresholds(self, mock_get, selector):
