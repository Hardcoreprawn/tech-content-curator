...
- **💰 Best Value:** `{best_value["model"]}` (${best_value["cost_per_1k_words"]:.4f} per 1K words)
- **⚡ Fastest:** `{fastest["model"]}` ({fastest["avg_elapsed_seconds"]:.1f}s average)
- **🎯 Most Token Efficient:** `{most_efficient["model"]}` ({most_efficient["avg_output_tokens"] / most_efficient["avg_word_count"]:.1f} tokens/word)
